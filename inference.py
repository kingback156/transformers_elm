...
def solve(args):
    ...
    for src_sent, tgt_sent in zip(src_data, tgt_data):
        ...
        print(
            f"Source:{remove_bpe(src_sent)}\nTarget:{' '.join(tgt_sent)}\nPredict:{remove_bpe(predict_sent)}\n"
        )

        with open("ab.txt", "a+") as fl:
            print("source:", remove_bpe(src_sent), file=fl)
            print("Target:", remove_bpe(' '.join(tgt_sent)), file=fl)
            print("Predict:", remove_bpe(predict_sent), file=fl)
            print(file=fl)
        continue
