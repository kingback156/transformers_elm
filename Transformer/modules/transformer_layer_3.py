...
class TransformerDecoderLayer(nn.Module):
    ...
    def decoder_svdchange(self, model_dim, ffn_dim): # All SVD operations can be done here
        ...
        self.diag_param = nn.Parameter(torch.ones(min(model_dim, ffn_dim))) # Create training diagonal parameters
    ...
    def forward(self,net_input: torch.Tensor,padding_mask: torch.Tensor,attn_mask: Optional[torch.Tensor] = None,
        ...
        # 构建仅对角线可训练完整的E矩阵
        E = torch.zeros(self.U.size(0), self.V.size(1), device=net_input.device) # 构建[model_dim, ffn_dim]全零矩阵
        min_dim = min(E.size(0), E.size(1))# 计算E矩阵哪个最小
        E[torch.arange(min_dim), torch.arange(min_dim)] = self.diag_param #在对角线上填充可训练参数
