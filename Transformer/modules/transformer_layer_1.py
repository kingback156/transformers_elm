...
class TransformerDecoderLayer(nn.Module):
    ...
    def decoder_svdchange(self, model_dim, ffn_dim): # All SVD operations can be done here
        ...
        E = torch.zeros(model_dim, ffn_dim)
        E[:model_dim, :model_dim] = torch.diag(S)
        self.E = nn.Parameter(E) # To summarize: first create a zero matrix, then put the previous one in the upper left corner
...
class TransformerEncoderLayer(nn.Module):
    ...
    def encoder_svdchange(self, model_dim, ffn_dim):# All SVD operations can be done here
        ...
        E = torch.zeros(model_dim, ffn_dim)
        E[:model_dim, :model_dim] = torch.diag(S)
        self.E = nn.Parameter(E) # as shown above
